...
import random
import json
from utils import clear_dir, euclidean_dist
import sys
...
loop = '''
_ , session_ID, iter_num = experiment_loop(session_ID, exp_num, int(50), iter_num, target_category)
'''

if __name__ == "__main__":
    # times = []
    # for i in range(10):
    #     time = timeit.timeit(setup=timing_setup, stmt=loop, number = 1)
    #     times.append(time)
    # print(times)
    setup_gpu = [1.2652260760078207, 1.2986614620313048, 1.122469809022732, 1.2570969359949231, 1.2451045799534768, 1.237137128948234, 1.2401893199421465, 1.2590895249741152, 1.2264192170696333, 1.2770511520793661]
    setup_cpu = [23.041716400068253, 23.231749205035158, 23.351712099043652, 23.378187207970768, 23.373018058016896, 23.119269696995616, 23.460189465084113, 23.439595978008583, 23.365848063956946, 23.348373066051863]
    loop_cpu = [23.831615800969303, 23.94933198799845, 24.164956636028364, 24.27726368000731, 24.020713714999147, 24.223133306019008, 24.44785244204104, 24.10523986304179, 24.103346571908332, 24.07070401101373]
    loop_gpu = [1.4374270580010489, 1.626458899001591, 1.4857782420003787, 1.4412244139239192, 1.491907718940638, 1.410928590921685, 1.4317005019402131, 1.62779659696389, 1.4587553220335394, 1.4761411759536713]

    sg = np.array(setup_gpu)
    sc = np.array(setup_cpu)
    lg = np.array(loop_gpu)
    lc = np.array(loop_cpu)

    print(f'GPU SETUP | mean {np.mean(sg)} | sd {np.std(sg)}')
    print(f'GPU LOOP  | mean {np.mean(lg)} | sd {np.std(lg)}')
    print(f'CPU SETUP | mean {np.mean(sc)} | sd {np.std(sc)}')
    print(f'CPU LOOP  | mean {np.mean(lc)} | sd {np.std(lc)}')

'''
GPU SETUP | mean 1.2428445206023753 | sd 0.04478636420133383
GPU LOOP  | mean 1.4888118519680575 | sd 0.07319745013767852
CPU SETUP | mean 23.310965924023186 | sd 0.12991233846514966
CPU LOOP  | mean 24.119415801402646 | sd 0.16430134803338445
'''
